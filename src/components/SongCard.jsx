    ...
    <div className={`relative group bg-white dark:bg-slate-900/90 rounded-2xl border transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 flex flex-col justify-between ${
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-3">
          ...
          {song.hasImage && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200/60 dark:border-purple-800/60">
              <ImageIcon className="w-3 h-3 text-purple-500" />
              <span>Foto JPG ({song.images.length})</span>
            </span>
          )}
          ...
          {song.hasPpt && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/60">
              ...
              <span>Slide PPT ({song.slides.length})</span>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="p-3 sm:px-5 sm:py-3 bg-slate-50/80 dark:bg-slate-850/50 rounded-b-2xl border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between gap-1.5 sm:gap-2">
        
        {/* Pulsante Play Audio (se disponibile) */}
        {mainAudio ? (
          <button
            onClick={() => onPlayAudio(song, mainAudio)}
            className={`flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold shadow-xs transition-all active:scale-95 ${
              isPlayingThisSong
        ...
        {/* Pulsante Spartito PDF o Immagine JPG */}
        {mainScore && (
          <button
            onClick={() => onOpenDocument(mainScore, song)}
            title={`Visualizza ${mainScore.cleanName}`}
            className="flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 border border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 transition-all"
          >
            {mainScore.fileType === 'image' ? (
              <ImageIcon className="w-3.5 h-3.5 text-purple-500" />
            ) : (
              <Eye className="w-3.5 h-3.5 text-emerald-500" />
            )}
            <span>{mainScore.fileType === 'image' ? 'Foto' : 'Spartito'}</span>
          </button>
        )}

        {/* Pulsante Presentazione PPT (Apre direttamente in App!) */}
        {mainSlide && (
          <button
            onClick={() => onOpenDocument(mainSlide, song)}
            title={`Visualizza presentazione slide ${mainSlide.cleanName}`}
            className="flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-slate-800 text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-950/40 border border-slate-200 dark:border-slate-700 hover:border-amber-300 transition-all"
          >
        ...
        <div className="relative">
          ...
          {showMenu && (
            <>
              ...
              />
              <div className="absolute right-0 bottom-full mb-1 w-72 p-2 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-30 space-y-1 text-xs">
